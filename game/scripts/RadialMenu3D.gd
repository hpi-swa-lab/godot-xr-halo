...
func generate_menu():
	...
	for i in range(count):
		...
		var slice_mesh = create_slice_mesh(start_angle, end_angle)
		...
		if i < option_icons.size() and option_icons[i] != null:
			add_icon(slice_obj, start_angle, end_angle, option_icons[i])

func create_slice_mesh(start_angle, end_angle) -> ArrayMesh:
...
func add_icon(parent, start, end, texture):
...
func confirm_selection():
	if selected_index >= 0 and selected_index < option_ids.size():
		...
		# Play sound based on selection type
